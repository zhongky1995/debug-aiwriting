...
它首先是一个通用中文写作调试 skill。方案、报告、白皮书、案例、邮件、社交内容和脚本会分别加载对应的场景规则；UGC/人物脚本只是其中一种按需使用的类型，不会反过来支配其他文体。
...
- 源稿存在重复脚本、重复标题或元数据缺口，却没有在批量改写前先识别和确定处理方式
...
- **源稿预检**：批量改写前先统计脚本、标题、人设、配音说明、结尾、整段重复和重复口播格；重复稿先确认是保留同步、合并删除，还是有足够事实支持分别改写。
- **人物锚点样稿**：每个主要人物组先改 3-5 条，隐藏全部标签后盲读通过，再扩到整批；不从配音标签直接批量生成口吻。
...
### 2026-07-19 补强：批量改写先检查源稿，再改口吻

本轮增加了批量脚本的前置工作顺序：

1. 先检查脚本数量、标题、人设、配音说明、结尾和重复稿，不能改到一半才发现大量源稿相同。
2. 重复稿先确定处理方式：保留时修改一份标准稿并同步；需要删除或合并时先确认；只有源稿确实提供了不同人物事实，才分别改写。
3. 每个主要人物组先做 3-5 条锚点样稿，隐藏人设和配音标签后检查台词，再扩大到整批。
4. 结尾不只查“有没有感悟句”，还统计结尾类型和开头短语的集中度，用于发现大批脚本仍以同一种方式收口。
5. 人设资料不足时明确报告限制，不为了显得每条都不同而补写未经提供的生活细节。

### 2026-07-19 补强：结尾不等于最后一个动作

新版区分“时间上最后发生的事”和“叙事真正落下来的位置”。人物已经说出关键反应后，再补等待、收拾、走向停车区等流程，往往会把结尾拖成流水账；最后一个镜头可以保留，但不必强行填口播。审计会把“还没、又、继续、等着”等未完成信号和离场、停车、收拾等尾部流程列为风险提示，最终仍由上下文判断。
...
报告会统计脚本数量、标题/人设/配音说明缺口、重复标题、配音说明集中度、整段重复脚本、重复口播格、结尾类型与开头短语集中度、未完成式结尾和流程拖尾风险、第一人称结论公式、隐性价值总结、不自然动宾、压缩式金句、固定反问结尾、旁白格填满情况，以及原稿中非空结尾被改稿清空的数量和位置。它用于定位批量问题，不代替逐条理解人物和场景；配音说明更丰富、结尾形式更分散，也不能单独证明台词已经自然。
