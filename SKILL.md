...
5. Apply the default personal voice profile from `references/zhihu-voice-profile.md` when the task is Chinese public writing, opinion writing, workplace/management/interview content, or when the user asks for "my writing/my tone." For formal client, report, or email contexts, keep the same decision logic but reduce bluntness and colloquial pressure.
6. For client-facing proposals or decks, rebuild the decision chain before line editing: client question -> diagnosis -> strategy -> mechanism -> execution/proof -> boundary. The goal is credible decision support, not a warmer service menu.
7. Apply a two-pass edit:
...
8. Run the concrete-language and verifiable-action gates. Any sentence with framework nouns but no actor, action, object, or result must be rewritten before output. Any sentence that turns a desired result into a fake action must be rewritten into something observable.
9. Output in the user's requested format. If no format is specified:
...
3. **External boundary**: remove or translate information the reader should not see.
4. **Personal voice**: apply the user's practical, direct, conditional reasoning style.
5. **Anti-AI cleanup**: remove template structure, empty abstractions, and false polish.
