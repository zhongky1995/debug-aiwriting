...
11. For large documents, make a page/section coverage ledger and edit by surface: document title, page titles, openings, tables, captions, footnotes, summary rows, and final notes. Use `scripts/audit_surfaces.py` on supported text sources to inventory surfaces and residual high-risk phrases. Do not stop after rewriting body paragraphs.
    - For script banks, run source-integrity preflight before rewriting: inventory every title, persona line, voice direction, full-script paragraph, shot table, voice-over cell, on-screen caption, and ending; report missing metadata, repeated titles, exact duplicate full scripts, repeated voice-over blocks, and concentrated ending shapes. Use `scripts/audit_ugc_scripts.py` for `.docx` script banks when possible.
    - Set a duplicate policy before editing. For intentional duplicates, edit one canonical copy and propagate the approved wording. For accidental duplicates, report and remove or consolidate only with approval. Never fabricate different life details merely to make duplicate records look unique.
    - For large persona families, rewrite 3-5 anchor scripts first and check the actual spoken lines with metadata hidden before scaling. Do not mass-edit the bank from labels or TTS directions alone.
...
- Do not accept age, gender, occupation, clothing, hairstyle, or TTS directions as evidence of tone diversity. Hide all metadata and read only the spoken lines. The lines themselves must differ in what the speaker notices, how quickly they move, how much they explain, what they leave unsaid, and how they judge the event.
- Treat voice direction, persona labels, and production notes as supporting metadata, not a substitute for copy changes. Do not invent missing age, identity, occupation, family role, or voice traits from the script. A missing voice direction is a metadata gap, not proof that the spoken line itself is bad.
- Build script voice on two axes: a stable speaker system plus the current scene mode. Do not force every mother into a rushed tone, every technical user into review copy, or every retired man into reflective prose.
...
- Do not turn that permission into blanket deletion. The final row has a narrative function even when it has no spoken takeaway. Decide the ending per script, never by persona group, content category, or index range. If a populated source ending becomes an empty final cell, treat it as a coverage failure unless the final image clearly completes the scene and intentional silence is explicitly marked in the production format.
...
- After removing a lesson-like final sentence, verify the literal final table row. Preserve or rewrite the source-supported last action, quote, reaction, or visual callback; do not let the script simply stop one row early. For large script banks, compare source and revised final cells and require `removed_nonempty_final_count = 0` unless every exception is documented as intentional silence.
...
- When the source is thin, say that stronger human texture or sharper persona differentiation requires more source material instead of inventing details. Report the limit rather than claiming the bank has been fully diversified.
...
- for character/UGC scripts: persona names and voice directions that look diverse while the actual lines remain interchangeable; one calm explanatory narrator spread across all roles; or exact repeated voice-over hidden behind different numbering, styling, or production notes
